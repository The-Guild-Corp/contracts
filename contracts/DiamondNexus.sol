...
import {NexusStorage} from "./facets/nexus/storage/NexusStorage.sol";

contract DiamondNexus {
    constructor(
        ...
        // Add Nexus facet
        IDiamondCut.FacetCut[] memory cutNexus = new IDiamondCut.FacetCut[](1);
        (bytes4[] memory nexusSelectors, ) = IFacet(_nexusFacet)
            .pluginMetadata();
        cutNexus[0] = IDiamondCut.FacetCut({
            facetAddress: _nexusFacet,
            action: IDiamondCut.FacetCutAction.Add,
            functionSelectors: nexusSelectors
        });
        LibDiamond.diamondCut(cutNexus, address(0), "");
